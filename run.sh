...
do
    ...
    # python3 src/main.py --config=qmix_think_tank_large --env-config=sc2 with env_args.map_name=$map_name  t_max=$t_max device_name=1 > /dev/null & 

    # export CUDA_VISIBLE_DEVICES=2
    # python3 src/main.py --config=qmix_think_tank_large --env-config=sc2 with env_args.map_name=$map_name  t_max=$t_max device_name=2 > /dev/null & 
    # wait
done
