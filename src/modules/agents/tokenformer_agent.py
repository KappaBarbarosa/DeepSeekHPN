...
class TokenformerAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(TokenformerAgent, self).__init__()
        self.args = args
        ...
        self.tokenformer = TokenformerLayer(
            neox_args=args,
            attention_mask_func=lambda x, y: x,
            init_method=init_method,  # 根據你的設定初始化
    ...
    def forward(self, inputs, hidden_state):
        ...
        h_in = h_in.unsqueeze(1)  # [bs, 1, rnn_hidden_dim]
        x = x.unsqueeze(1)  # [bs * n_agents, 1, rnn_hidden_dim]
        # print('x', x[0])
        # print ("tokenformer input shape: ", x.shape, h_in.shape)

        attention_mask = self.build_causal_attention_mask(x.size(1), x.size(0), x.device)
        # print ("attention_mask shape: ", attention_mask.shape)
        tokenformer_out, new_hh = self.tokenformer(x, attention_mask, hh=h_in)
        # print(f'tokenformer_out', tokenformer_out[0])
        # print('new_hh', new_hh[0])
        # exit() 
        # print ("tokenformer output shape: ", tokenformer_out.shape, new_hh.shape)
        new_hh = new_hh.squeeze(1)  # [bs * n_agents, rnn_hidden_dim]
        # Q-values of normal actions
        q_normal = self.fc2_normal_actions(new_hh).view(bs, self.n_agents, -1)  # [bs, n_agents, 6]
        # print ("q_normal shape: ", q_normal.shape)
        ...
        if self.args.map_type == "MMM":
            ...
            q_rescues = (th.matmul(tokenformer_out.unsqueeze(1), fc2_w_rescue).squeeze(1) + fc2_b_rescue).view(
        ...
        return q.view(bs, self.n_agents, -1), new_hh.view(bs, self.n_agents, -1)  # [bs, n_agents, 6 + n_enemies]

    def build_causal_attention_mask(self, seq_len, batch_size, device):
        # 建立 [seq_len+1, seq_len+1] 的下三角矩陣
        base_mask = th.tril(th.ones(seq_len+1, seq_len+1, dtype=th.bool, device=device))
        # 擴展成 [batch_size, seq_len+1, seq_len+1]
